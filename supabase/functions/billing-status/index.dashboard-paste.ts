...
/* Supabase Dashboard: slug billing-status; add secret DONNA_BILLING_TOKEN; disable JWT verify. */
...
async function tokensMatchConstantTime(a: string, b: string): Promise<boolean> {
...
function mapAccountStatus(raw: string): "active" | "trialing" | "past_due" | "canceled" {
  switch (raw) {
    case "active":
      return "active"
    case "trialing":
...
function parseBillingBody(json: unknown): { ok: true; body: BillingBody } | { ok: false; status: 400 } {
...
function sandboxResponseForEmail(email: string): Response | null {
...
async function handleBillingRequest(req: Request, env: BillingEnv): Promise<Response> {
